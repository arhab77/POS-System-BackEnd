...
const store = async(req, res, next) => {
    try{
        //mengambil form name, description, dan price
        let payload = req.body;

        //upload dengan relasi kategory
        if(payload.category){
        ...
        if(req.file){
            ...
            src.on('end', async() => {
                try{
                    //jika berhasil semua form digabung
                    let product = new Product({...payload, image_url: filename})
                    await product.save()
                    return res.json(product);
                } catch(err) {
                    //jika error dengan pesan error validation
            ...
            //upload ketika error akan mengirimkan erornya
            src.on('error', async() => {
                next(err);
            });
        } else {
    ...
    try {
        ...
        if (req.file) {
            ...
            src.on('error', async (err) => {
                // handle error ketika upload image
                fs.unlinkSync(target_path); // hapus target file jika terdapat error
                next(err);
            });
            ...
            src.on('end', async () => {
                try {
                    ...
                    // Jika error dengan pesan error validasi
                    fs.unlinkSync(target_path);
            ...
            src.pipe(dest); // Mulai process upload image
